...
void part1(int x) {
	...
	int result;
	result = isPositive(x++); // chnaged from ++x to x++

	printf("isPositive(10) = %d \n\n", result);
	
	// Why did this error occur? Please provide the answer in your own words below.
	printf("Explanation: while calling macros with operators like '++'; operators might get evaluated multiple times. Hence if we increment value before (++x) we will get a value of +2 hence 11. If we increment after (x++) the program will increment after first execution so, increment by 1; hence 10.\n\n\n"); // (2.5 points)
}
...
void part2(int x, int y) {
	...
	printf("In VS : the result of polyFunc = 36 and polyMacro = 28 \n"); 			// (5 points)
	printf("In GCC: the result of polyFunc = 36 and polyMacro = 25 \n\n"); 		// (5 points)
	...
	printf("Explanation: Since the order and meathod in which commands are executed by different compliers are different; different compliers will give different results. \n\n"); //  (2.5 points)
}
